...
impl MetalRenderer {
    ...
    fn draw_frame_buffer(&mut self, RenderTargetId(source_buffer): RenderTargetId, region: FrameBufferRegion, alpha: f64, state: &mut RenderState) {
        ...
        if let Some(source_buffer) = &render_targets[source_buffer] {
            ...
            let triangle_strip              = vec![
                Vertex2D { pos: [ min_x, min_y ],                   tex_coord: [ min_x, source_height-min_y ],                color: [0,0,0,0] },
                Vertex2D { pos: [ min_x, max_y ],                   tex_coord: [ min_x, source_height-max_y ],                color: [0,0,0,0] },
                Vertex2D { pos: [ max_x, min_y ],                   tex_coord: [ max_x, source_height-min_y ],                color: [0,0,0,0] },
                Vertex2D { pos: [ max_x, max_y ],                   tex_coord: [ max_x, source_height-max_y ],                color: [0,0,0,0] },
            ];
    ...
    fn use_shader(&mut self, shader_type: ShaderType, state: &mut RenderState) {
        ...
        match shader_type {
            ShaderType::DashedLine { dash_texture: _, clip_texture: _ } => {
