...
pub async fn macos_platform_subprogram(input: InputStream<WinitEvents>, _context: SceneContext, winit_events: Subscriber<WinitEvents>) {
    ...
    while let Some(event) = winit_events.next().await {
        match event {
            WinitEvents::CreatedWindow { window_id, scale, events, platform_window, ready } => {
                // For WGPU windows, we attach a view that handles tablet events and suppresses any animations generated by OS X
                #[cfg(feature="render-wgpu")]
                {
