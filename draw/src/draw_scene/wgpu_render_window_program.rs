...
pub fn create_wgpu_render_window_program(scene: &Arc<Scene>, program_id: SubProgramId, initial_size: (u64, u64)) -> Result<(), ConnectionError> {
    ...
    scene.add_subprogram(program_id, move |render_window_requests, context| {
    ...
    scene.connect_programs((), StreamTarget::Filtered(FILTER_RENDER_REQUEST.clone(), program_id), StreamId::with_message_type::<RenderRequest>().for_target(program_id)).unwrap();
    scene.connect_programs((), StreamTarget::Filtered(FILTER_EVENT_WINDOW_REQUEST.clone(), program_id), StreamId::with_message_type::<EventWindowRequest>().for_target(program_id)).unwrap();
