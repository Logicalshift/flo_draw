...
///
/// Renders a whole frame of pixels to a ARGB U32 buffer (using TPixel as the intermediate format)
///
pub struct U32ArgbFrameRenderer<TPixel, TRegionRenderer>
where
    TPixel:             Sized + Send + Default + ToGammaColorSpace<U32ArgbPremultipliedPixel>,
...
impl<TPixel, TRegionRenderer> U32ArgbFrameRenderer<TPixel, TRegionRenderer>
where
    TPixel:             Sized + Send + Clone + Default + ToGammaColorSpace<U32ArgbPremultipliedPixel>,
    TRegionRenderer:    Renderer<Region=RenderSlice, Dest=[TPixel]>,
{
    ///
...
impl<'a, TPixel, TRegionRenderer> Renderer for U32ArgbFrameRenderer<TPixel, TRegionRenderer> 
where
    TPixel:             Sized + Send + Clone + Default + ToGammaColorSpace<U32ArgbPremultipliedPixel>,
    TRegionRenderer:    Renderer<Region=RenderSlice, Dest=[TPixel]>,
{
    ...
    type Dest   = [U32ArgbPremultipliedPixel];

    fn render(&self, region: &GammaFrameSize, source: &TRegionRenderer::Source, dest: &mut [U32ArgbPremultipliedPixel]) {
