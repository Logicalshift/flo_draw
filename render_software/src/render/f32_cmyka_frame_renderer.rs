...
type TPixel = F32CmykaPixel;

///
/// Renders a whole frame of pixels to a RGBA U8 buffer (using TPixel as the intermediate format)
///
pub struct F32CmykaFrameRenderer<TRegionRenderer>
where
    // TPixel:             Sized + Send + Default + F32CmykaPixel, // ToGammaColorSpace<U8RgbaPremultipliedPixel>,
...
impl<TRegionRenderer> F32CmykaFrameRenderer<TRegionRenderer>
where
...
impl<'a, TPixel, TRegionRenderer> Renderer for F32CmykaFrameRenderer<TRegionRenderer> 
where
    TPixel:             Sized + Send + Clone + Default + ToGammaColorSpace<U8RgbaPremultipliedPixel>,
    TRegionRenderer:    Renderer<Region=RenderSlice, Dest=[TPixel]>,
{
...
impl<'a, TRegionRenderer> Renderer for F32CmykaFrameRenderer<TRegionRenderer> 
where
    // TPixel:                     Sized + Send + Clone + Default + ToGammaColorSpace<U8RgbaPremultipliedPixel>,
    TRegionRenderer:            Renderer<Region=RenderSlice, Dest=[TPixel]>,
{
    type Region = GammaFrameSize;
    type Source = TRegionRenderer::Source;
    type Dest   = [F32CmykaPixel];

    fn render(&self, region: &GammaFrameSize, source: &TRegionRenderer::Source, dest: &mut [F32CmykaPixel]) {
        const LINES_AT_ONCE: usize = 8;
        ...
        chunks.enumerate().map(|(chunk_idx, chunk)| {
            ...
            |(ref mut render_slice, ref mut buffer), (y_positions, _chunk_idx, chunk)| {
                ...
                // Render these lines
                renderer.render(render_slice, source, chunk);
