...
impl TextureReader<RgbaTexture> for F32CmykaPixel {
	#[inline]
	...
	fn read_pixels(texture: &RgbaTexture, positions: &[(f64, f64)]) -> Vec<Self> {
		...
		pixels.extend(u8pixels.map(|[r, g, b, a]| {
			...
			F32CmykaPixel::from_components([rf, gf, bf, 0.0, af])
		}));
...
impl TextureReader<U16LinearTexture> for F32CmykaPixel {
	#[inline]
	...
	fn read_pixels(texture: &U16LinearTexture, positions: &[(f64, f64)]) -> Vec<Self> {
		...
		// Convert to F32CmykaPixels
		pixels.extend(u16pixels.map(|[r, g, b, a]| F32CmykaPixel::from_components([*r as f32/65535.0, *g as f32/65535.0, *b as f32/65535.0, 0.0, *a as f32/65535.0])));
