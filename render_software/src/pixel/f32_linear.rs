...
impl ToGammaColorSpace<U32ArgbPremultipliedPixel> for F32LinearPixel {
    fn to_gamma_colorspace(input_pixels: &[F32LinearPixel], output_pixels: &mut [U32ArgbPremultipliedPixel], gamma: f64) {
        ...
        for (input, output) in input_pixels.iter().zip(output_pixels.iter_mut()) {
            ...
            *output = U32ArgbPremultipliedPixel::from_rgba_components(
                gamma_lut.look_up(r as _), 
                gamma_lut.look_up(g as _), 
                gamma_lut.look_up(b as _), 
                (a >> 8) as u8);
...
impl ToLinearColorSpace<U16LinearPixel> for F32LinearPixel {
    fn to_linear_colorspace(input_pixels: &[Self], output_pixels: &mut [U16LinearPixel]) {
        let f32x4_65535 = f32x4::splat(65535.0);

        for (f32_pixel, u16_pixel) in input_pixels.iter().zip(output_pixels.iter_mut()) {
            // Convert to a set of U32 values between 0-65535
            let rgba = f32_pixel.0.min(f32x4::ONE).max(f32x4::ZERO);
            let rgba = rgba * f32x4_65535;
            let rgba = rgba.fast_trunc_int();
...
impl AlphaBlend for F32LinearPixel {
    ...
    #[inline] fn source_over(self, dest: Self) -> Self        { let src_alpha = self.0.as_array()[3]; F32LinearPixel(self.0 + dest.0*(1.0-src_alpha)) }
    #[inline] fn dest_over(self, dest: Self) -> Self          { let dst_alpha = dest.0.as_array()[3]; F32LinearPixel(self.0*(1.0-dst_alpha) + dest.0) }
    #[inline] fn source_in(self, dest: Self) -> Self          { let dst_alpha = dest.0.as_array()[3]; F32LinearPixel(self.0*dst_alpha) }
    #[inline] fn dest_in(self, dest: Self) -> Self            { let src_alpha = self.0.as_array()[3]; F32LinearPixel(dest.0*src_alpha) }
    #[inline] fn source_held_out(self, dest: Self) -> Self    { let dst_alpha = dest.0.as_array()[3]; F32LinearPixel(self.0*(1.0-dst_alpha)) }
    #[inline] fn dest_held_out(self, dest: Self) -> Self      { let src_alpha = self.0.as_array()[3]; F32LinearPixel(dest.0*(1.0-src_alpha)) }
