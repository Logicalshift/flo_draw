    ...
    TTexture: Send + Sync,
{
    ...
    ///
    /// This reads `count` pixels at locations `t = 0, 1, 2, ...` such that `u = dx * t` and `x = x_gradient.0 * u + x_gradient.1`,
    /// `y = y_gradient.0 * u + y_gradient.1`.
    ///
    #[inline]
    fn read_pixels_linear(texture: &TTexture, dx: f64, x_gradient: (f64, f64), y_gradient: (f64, f64), count: usize) -> Vec<Self> {
