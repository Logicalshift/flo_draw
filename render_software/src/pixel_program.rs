...
pub enum PixelProgramPlan {
    ...
    /// generated by blending. This is useful for anti-aliasing spans that are smaller than a pixel
    Blend(usize, f32),

    /// As for Blend but the blend changes linearly between the first and the last value over the region covered by the
    /// span (this is useful for anti-aliasing over longer regions)
    LinearBlend(usize, f32, f32),
}
