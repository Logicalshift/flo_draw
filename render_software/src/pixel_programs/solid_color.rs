...
impl<TPixel: Copy + Send + Sync> PixelProgram for SolidColorProgram<TPixel> {
    ...
    #[inline]
    fn draw_pixels(&self, _data_cache: &PixelProgramDataCache<Self::Pixel>, target: &mut [Self::Pixel], x_range: Range<i32>, _: Range<f64>, _y_pos: f64, program_data: &Self::ProgramData) {
    ...
    TPixel: AlphaBlend
{
    ...
    fn draw_pixels(&self, _data_cache: &PixelProgramDataCache<Self::Pixel>, target: &mut [Self::Pixel], x_range: Range<i32>, _: Range<f64>, _y_pos: f64, program_data: &Self::ProgramData) {
        for pixel in target[(x_range.start as usize)..(x_range.end as usize)].iter_mut() {
            *pixel = pixel.source_over(program_data.0);
        }
    }
}
