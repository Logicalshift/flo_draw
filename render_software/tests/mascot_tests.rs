//
// These tests are against the mascot data: they look at glitches that were found there that didn't
// have an obvious simpler reproduction
//
    ...
    TScanPlanner: ScanPlanner<Edge=Arc<dyn EdgeDescriptor>>,
{
    ...
    gc.extend(mascot_drawing_from_paths());
    ...
    for idx in 1..scanlines.len() {
        assert!(scanlines[idx].1 == first_line.1, "Scanline {:?} doesn't match\n\n({:?}\n\n!=\n\n{:?})", idx, first_line, scanlines[idx]);
    }
...
///
/// Mascot drawing instructions
///
fn mascot_drawing() -> Vec<Draw> {
    decode_drawing(MASCOT.chars()).collect::<Result<Vec<Draw>, _>>().unwrap()
}

///
/// Mascot decoded as path instructions
///
fn mascot_paths() -> Vec<(Vec<PathAttribute>, Vec<SimpleBezierPath>)> {
    let mascot = mascot_drawing();
...
///
/// Mascot decoded to paths and then re-encoded as a drawing (strips out the canvas setup)
///
fn mascot_drawing_from_paths() -> Vec<Draw> {
    let mut gc = vec![];

    for (attributes, path_set) in mascot_paths() {
