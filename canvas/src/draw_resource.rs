...
pub (crate) enum DrawResource {
    ...
    Texture(NamespaceId, TextureId),
    Gradient(NamespaceId, GradientId),
    Font(NamespaceId, FontId),
    FontSize(NamespaceId, FontId),
...
impl Draw {
    ...
    ///
    #[inline]
    pub (crate) fn uses_resource(&self, resource: &DrawResource, namespace: &NamespaceId) -> bool {
        use self::Draw::*;
        ...
        match self {
            ...
            Texture(texture_id, TextureOp::CreateDynamicSprite(sprite_id, _, _)) => {
                if resource == &DrawResource::CanvasTransform {
                    true
                } else if resource == &DrawResource::Sprite(*namespace, *sprite_id) {
                    true
                } else if resource == &DrawResource::Texture(*namespace, *texture_id) {
                    true
                } else {
                    false
                }
            ...
            Texture(texture_id, _)                  => resource == &DrawResource::Texture(*namespace, *texture_id),
            Gradient(gradient_id, _)                => resource == &DrawResource::Gradient(*namespace, *gradient_id),
            Font(font_id, FontOp::LayoutText(_))    |
            Font(font_id, FontOp::DrawGlyphs(_))    => match resource { 
                DrawResource::Font(resource_namespace, resource_font_id) | DrawResource::FontSize(resource_namespace, resource_font_id) => font_id == resource_font_id && namespace == resource_namespace,
                DrawResource::CanvasTransform | DrawResource::FillWindingRule | DrawResource::FillBlend | DrawResource::FillColor => true,
                _ => false
            },

            DrawSprite(sprite_id)                   => resource == &DrawResource::CanvasTransform || resource == &DrawResource::Sprite(*namespace, *sprite_id),
            ...
            FillTexture(texture_id, _, _)           => resource == &DrawResource::Texture(*namespace, *texture_id),
            FillGradient(gradient_id, _, _)         => resource == &DrawResource::Gradient(*namespace, *gradient_id),
